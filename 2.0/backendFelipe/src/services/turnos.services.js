...
// POST (con validación de solapamiento 🔥)
const crearTurno = async (data) => {
  ...
  const existe = await pool.query(
    ...
    [fecha, profesional_id, hora_inicio, hora_fin]
  );
...
const actualizarTurno = async (id, data) => {
  const { fecha, hora_inicio, hora_fin, paciente_id, profesional_id, consultorio_id, especialidad_id, motivo_consulta, estado } = data;

  const result = await pool.query(
    `UPDATE turnos SET fecha=$1, hora_inicio=$2, hora_fin=$3, paciente_id=$4, profesional_id=$5,
     consultorio_id=$6, especialidad_id=$7, motivo_consulta=$8, estado=$9,
...
module.exports = { obtenerTurnos, obtenerTurnoPorId, crearTurno, actualizarTurno, eliminarTurno };